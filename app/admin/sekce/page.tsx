...
export const metadata: Metadata = { title: 'Admin â€“ Sekce' }
...
export default async function AdminSekce({
  ...
  // RozdÄ›lenĂ­ na top-level sekce a podsekce
  ...
  // Sekce pro dropdown (jen top-level â€” nelze vybrat sebe sama ani podsekci jako rodiÄŤe podsekce)
    ...
    return (
      <div className="border-t border-gray-100 px-5 py-4">
        <form action={saveSection} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">NĂˇzev</label>
            <input name="title" type="text" required defaultValue={section.title}
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
          </div>

          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">NĂˇzev v menu (volitelnĂ˝)</label>
            <input name="menu_title" type="text" defaultValue={section.menu_title ?? ''}
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
          </div>

          <div className="sm:col-span-2">
            <label className="block text-xs font-medium text-gray-600 mb-1">NadĹ™azenĂˇ sekce</label>
            <select name="parent_id" defaultValue={parentId}
              ...
              <option value="">â€” HlavnĂ­ sekce (bez rodiÄŤe) â€”</option>
              {parentOptions
            ...
            <p className="mt-1 text-[11px] text-gray-400">Podsekce se v menu zobrazujĂ­ jako poloĹľky podmenu nadĹ™azenĂ© sekce.</p>
          </div>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">PoĹ™adĂ­ v menu</label>
          ...
          <div className="flex items-center gap-6 pt-4">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-sm text-gray-700">AktivnĂ­</span>
            </label>
          </div>
      ...
      >
        <summary className="flex items-center justify-between px-5 py-4 cursor-pointer select-none list-none">
          <div className="flex items-center gap-3">
            {indent && <span className="text-gray-300 text-xs" aria-hidden="true">â†ł</span>}
    ...
    <div>
      <div className="mb-6 flex items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">SprĂˇva navigaÄŤnĂ­ch sekcĂ­ a jejich nastavenĂ­.</p>
        </div>
      </div>

      {/* FormulĂˇĹ™ pro novou sekci */}
      <form action={createSection} className="mb-6 grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
        <div className="sm:col-span-2">
          <label className="block text-xs font-medium text-gray-600 mb-1">NĂˇzev novĂ© sekce</label>
          <input
            name="title"
            type="text"
            required
            placeholder="NĂˇzev novĂ© sekceâ€¦"
            className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-500"
          />
        </div>
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-1">NadĹ™azenĂˇ sekce (volitelnĂˇ)</label>
          <select name="parent_id"
            className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-500 bg-white">
            <option value="">â€” HlavnĂ­ sekce â€”</option>
            {parentOptions.map(p => (
              <option key={p.id} value={p.id}>{p.title}</option>
            ))}
          </select>
      ...
      {/* Seznam sekcĂ­ â€” top-level se svĂ˝mi podsekcemi */}
      <div className="space-y-4">
        {topLevel.map(section => (
          <div key={section.id} className="space-y-2">
            <SectionCard section={section} />
            {/* Podsekce pod touto sekcĂ­ */}
        ...
        {/* OsiĹ™elĂ© podsekce (parent byl smazĂˇn â€” parent_id existuje ale nedohledatelnĂ˝) */}
        {allSections
